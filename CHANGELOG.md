...
- **`/memory rm <번호>` — 메모리 한 줄만 삭제.** 지금까지는 `/memory clear` 로 통째로 비우고
  ...
  몰아서 하게 되는 일이라 하나씩 지우면 번호가 매번 밀려 목록을 다시 봐야 한다. 하나라도 범위를
  벗어나면 통째로 거절한다 — 일부만 지우고 나면 남은 번호가 어긋나서 이어 친 명령이 엉뚱한 규칙을
  지운다. 삭제 응답에는 **진행 중인 대화에는 한동안 남는다**는 안내를 붙였다 (`/memory clear` 도 동일) —
