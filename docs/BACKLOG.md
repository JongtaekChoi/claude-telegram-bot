...
- [ ] **방별 페르소나** — 역할을 프로세스가 아니라 방으로 가른다. 세션 시작 때 페르소나를 고르고,
      봇을 합칠지는 프로젝트마다 고른다(강제 아님). **1·3단계는 0.4.13 에서 끝났다** — 남은 건
      2(세션 경계의 선택 버튼) · 4(`/sessions` 필터) · 5(`/tell` 마커) · 6(작업 폴더 +
      `permissionMode`). 2 부터 이어서 한다.
...
- [ ] 방별 페르소나 1단계 — `personas` 넣은 봇에서 `/persona` 목록·지목, 세션 살아 있을 때 거절,
      `/new` 후 변경, 역할별 메모리, `ctb` 가 같은 역할로 뜨는지 (0.4.13)
