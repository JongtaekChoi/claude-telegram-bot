...
- [ ] **낡은 테스트 하네스 둘 복구** — `memory-test.mjs`·`sessions-test.mjs` 가 `bot.mjs` 에서
      블록을 떼는 앵커가 0.5.0 작업 때 밀려서 `SyntaxError` 로 죽는다(수정 전 `HEAD` 에서도 동일).
      **지금 이 둘은 커버리지가 0 이다** — 통과처럼 보이지 않고 죽어서 눈에 띄는 게 그나마 다행이다.
