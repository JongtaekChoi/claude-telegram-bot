...
- [ ] **자격증명 갈라짐 알림 + 실패 문구에 CLI 이름** — 부팅 검사는 실봇 확인 완료. `com.claudebot.dev` 를
      `kickstart -k` 로 재시작했더니 부팅에서 `Claude credentials: split` 을 잡고 오너 DM 을 보냈다
      (실제로 갈라진 상태였다). 하네스 27건 통과. **남은 것:** 인증 실패 답장 문구(`errClaudeAuth`)를
      방에서 눈으로 보는 것 — 지금 자격증명이 갈라져 있어 아무 방에나 말을 걸면 바로 나온다.
- [ ] **건너뛴 예약 작업 알림** — 하네스로 20건 확인했다(원인 구분, 종료 버튼, 한 시간 1회 상한,
      뮤트 방 제외, `chat` 유무별 목적지, 락 파일 경합). **실봇에서 볼 것:** 로컬 `ctb` 를 켜둔 채
      예약 작업을 발사시켜 그 방에 알림이 오는지, **종료 버튼이 실제로 로컬 세션을 끝내는지**
      (`send()` + 인라인 버튼이라 배포 전 실동작 확인이 필수인 자리), 두 번째 발사 때 조용한지.
...
- [ ] **테스트 하네스를 레포로 들인다** — 지금 하네스는 전부 세션 스크래치패드에 있고 **세션이
      끊기면 사라진다.** 2026-09-02 에 skipnotice(20) · cron-chat(19) · persona(52) · memory(37) ·
      sessions(17) · lock · title · pick · compact-defer(12) 가 한꺼번에 증발했다. 매번 다시 쓰는 게
      아니라 `tests/` 에 두고 `npm test` 가 부르게 한다. 지금 `npm test` 는 `node --check` 뿐이라
      **문법 오류만 잡고 동작은 하나도 안 본다.**
      복원할 때 앵커 방식(`src.indexOf`)도 손본다 — `bot.mjs` 가 바뀔 때마다 조용히 깨진다.
