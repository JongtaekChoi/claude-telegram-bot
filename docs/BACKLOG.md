...
2026-09-09 에 남은 항목의 **로직은 하네스로 덮었다** (`/allow` 38건 · 포럼 접수처 38건 ·
`personas[].dir` 23건). 하네스가 대신 못 하는 것 — 텔레그램 왕복과 눈으로 보는 것 — 만 남았다.
봇 토큰으로는 메시지를 **보낼** 수만 있고 사용자로 **칠** 수는 없어서(dispatch 소켓도 슬래시
명령 파싱을 안 거친다) 이 칸은 사람이 한 번 훑어야 비워진다.
...
- [ ] **`personas[].dir` (6a)** — 임시 config 로 부팅·작업 감시까지는 확인했다(폴더 두 개에
      ...
      ⚠️ **지금은 확인할 방이 없다** — 이 봇에도 큐브 봇에도 `dir` 을 쓰는 역할이 없다. 한 봇이
      두 프로젝트를 맡게 될 때가 확인할 때다. **그때까지 이 항목은 배포를 막지 않는다.**
- [ ] **`/rooms`** — 새 명령. 하네스 31건으로 덮었다(막는 자리 셋 포함: 지금 방·도는 방·범위에
      섞인 경우). **실봇에서 볼 것:** `/rooms` 목록에 번호·⏳·`←`(지금 방)가 제대로 붙는지,
      `/rooms rm <번호>` 로 지운 방이 `/tell` 목록과 포럼 접수처 버튼에서도 같이 사라지는지,
      지운 방에서 다시 말을 걸면 새로 등록되는지, 그 방에서 `/sessions` 로 옛 세션을 되찾을 수
      있는지, 지금 방을 지우려 하면 막히는지. 큐브 봇의 "새 대화 …" 셋이 실제 대상이다.
      **`/rooms sweep` 도 같이 볼 것:** 살아 있는 토픽에서 점 하나가 깜빡였다 사라지는지(남으면
      안 된다), 지운 토픽만 빠지는지, 확인 못 한 방이 보고에 뜨는지. 텔레그램에서 토픽을 하나
      실제로 지우고 돌려봐야 진짜 확인이다 — 없는 thread id 로는 그 경로를 못 밟는다.
...
- [ ] **잃어버린 스위트 복원** — `tests/` 골격은 섰고 0.5.0 이 건드린 곳은 다 덮었다(286건).
