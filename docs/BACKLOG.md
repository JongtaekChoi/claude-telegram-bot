...
구현·커밋됐지만 실봇으로 확인하지 않은 것. 배포 전에 비어 있어야 한다.

- [ ] 메모리 봇별 분리 — cube 두 방에서 `/memory` 가 각자 것만 보여주는지, 한쪽에서 지운 게
      다른 쪽에 안 번지는지, `ctb` 가 봇과 같은 파일을 읽는지 (0.4.13)
      (부팅 이주는 확인함 — cube 기획자 재시작 때 `config.planner.memory.md` 생성, 원본 유지)
