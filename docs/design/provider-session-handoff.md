...
> **정정 (0.4.13):** "provider 세션만" 이 **현재 provider 하나만** 으로 구현돼 있었다. 그러면 살아남은
> Codex 세션을 다음 폴백이 그대로 이어받아, 맥락을 비웠다고 안내해 놓고 폴백만 옛 대화를 기억한다.
> 이제 `/new` 는 **양쪽 provider 세션을 다 지운다.** 방 단위인 건 그대로다 — 다른 방은 건드리지 않는다.
> 아울러 `/new` 는 실행 중에도 먹혀야 해서 busy 검사보다 앞에서 처리되는데, 그때 돌던 실행이 끝나며
> 후속 세션 ID 를 등록해 맥락이 되살아나는 문제가 있었다. 방 런타임에 세대(`gen`)를 두고 실행 시작
> 시점과 다르면 결과 세션을 버린다(`commitSid`).
