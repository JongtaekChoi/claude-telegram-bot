...
| `personas` | (선택) 역할 **목록**. 봇 하나가 여러 역할을 맡습니다. 항목마다 `id`(`[a-z0-9-]`, 파일명이 됩니다)와 `prompt` 가 필요하고, `name`·`model`/`provider`·`dir`(그 역할의 작업 폴더, `projectDir` 기준)은 선택. 방은 세션 경계에서 고르며, 안 고른 방은 첫 항목으로 돕니다. 아래 참고 |
...
| `roomRelay` | (선택) `/tell` 과 에이전트 마커로 이 봇이 맡은 다른 방에 메시지를 넘기는 기능 (기본값: `true`). `false`면 기능 전체를 끔 |
| `cliDispatch` | (선택) 로컬 유닉스 소켓(`.claude-bot/ctb.sock`, 0600)을 열어 `ctb send` 가 돌고 있는 봇에 메시지를 넘길 수 있게 함 (기본값: `true`). `false`면 소켓을 안 엶 |
...
| `/new` | 대화 세션 초기화 — Claude·Codex 양쪽. 폴백이 옛 맥락을 이어받지 못하게 한다 |
| `/newchat [이름]` *(= `/newtopic`)* | 이 그룹에 **새 주제(토픽)** 를 만들고 거기서 새 세션 시작 (주제 기능 켜짐 + 봇의 *주제 관리* 권한 필요) |
| `/sessions` | **이 방의 작업 폴더**에 쌓인 지난 세션 목록(활성 provider 기준) · 골라서 이어가기 (🔒 다른 방이 쓰는 중, 💻 터미널에서 열려 있음 — 둘 다 못 고름. `personas` 를 쓰면 이 방 역할의 세션만 보이고 ❓ 는 역할 생기기 전 세션) |
| `/name <이름>` | 지금 세션에 이름 붙이기 — `/sessions` 목록에서 바로 찾을 수 있다 (`/name -` 로 해제) |
| `/jobs` | 답장 후에도 살아 있는 백그라운드 작업 — ▶ 실행 중, ✅ 끝난 것 |
| `/persona` | 이 방이 어떤 역할로 도는지와 그 프롬프트 본문 (보기 전용 — 바꾸는 건 `/new` 의 버튼) |
| `/tell [방] [메시지]` | 이 봇이 맡은 다른 방으로 메시지를 넘깁니다 — 그 방의 세션으로 그 방에서 실행됩니다. 인자 없이 보내면 방 목록 |
| `/plan <요청>` | 계획 작성 후 승인 대기 (Claude 전용) |
| `/plan on` · `/plan off` | 이 방을 plan 모드로 고정 — 모든 메시지가 계획부터 (Claude 전용) |
| `/compact` | 현재 컨텍스트 압축 (Claude 전용) |
| `/stop [--reset]` | 작업 중단, 선택적으로 이전 세션 ID 복원 |
| `/local [kill]` | 로컬 `ctb` 세션이 어느 방을 잡고 있는지 확인·텔레그램에서 종료 |
...
| `/allow` | 이 봇을 쓸 수 있는 방 목록(출처 표시) · `/allow <chatId>` 로 추가(재시작 없이 즉시 발효) · `/allow rm <chatId>` 로 제거. **오너만** — 게이트는 방이 아니라 본인 텔레그램 user id 이고, 에이전트와 `ctb send` 는 이 경로에 닿지 못합니다. `[config]` 출신은 여기서 못 지웁니다(파일을 고치고 `/restart`) |
...
> **`/plan on`** 은 그것을 방에 고정합니다. `/plan off` 를 보내기 전까지 이 방의 모든 메시지가 계획부터 세우고 같은 승인 버튼과 함께 돌아오므로, 매번 `/plan` 을 앞에 붙였는지 기억하지 않아도 됩니다. `provider`·`model` 과 같은 자리에 저장되는 방 설정이라 재시작 후에도 유지되고, 포럼에서 한쪽 토픽만 고정해 둘 수도 있습니다. **승인 실행은 고정을 무시하고** 봇의 실제 `permissionMode` 로 돕니다 — 안 그러면 승인한 계획을 또 계획하게 됩니다. 두 가지만 알아두면 됩니다: plan 모드는 Claude 전용이라 Codex 방에서는 고정이 놀고 있고(`/status` 에 표시됩니다), 고정 중에는 Claude 한도 초과 시 **Codex 로 폴백하지 않습니다** — 계획만 받으려던 요청이 파일을 고치는 실행으로 바뀌기 때문입니다.
...
> **`/local`** 은 로컬 `ctb` 터미널 세션이 어느 방을 잡고 있는지(방 이름·PID·실행 경과 시간) 보여주고, 종료 버튼을 함께 보냅니다. 데스크탑에 `ctb`를 켜둔 채 나와서 그 방이 "이 방을 로컬 세션이 잡고 있습니다"로만 답할 때 쓰는 용도입니다 — 다른 방은 그대로 도니까 두고 온 그 방만 막힙니다. 버튼을 누르면 그 세션의 **프로세스 그룹**에 `SIGTERM`을 보냅니다 — 터미널에서 `Ctrl-C`를 누른 것과 같은 경로라 `ctb`가 락을 정상적으로 풀고 인수인계 메시지까지 보냅니다. 봇 작업이 없을 때 `/stop`을 쓰면 같은 버튼이 나옵니다. 버튼 없이 바로 끝내려면 `/local kill`.
...
- **상위 토픽은 실행하지 않고 묻습니다** — 포럼 그룹은 열면 기본으로 상위 토픽("모두" 뷰)에 서고, 거기서 쓴 답장은 읽고 있던 토픽이 아니라 *상위* 세션으로 갑니다. 부주의가 아니라 텔레그램 포럼 UI 의 구조라 "조심하자"로는 안 고쳐집니다. 그래서 포럼 그룹에 봇이 아는 토픽이 생기면, 상위 토픽은 프롬프트를 실행하지 않고 **어디로 보낼지 버튼으로 묻습니다** — 형제 토픽마다 버튼 하나, 그리고 `▶ 여기서 실행` 과 `❌ 안 보냄`. 누르기 전에는 아무것도 돌지 않으므로, 실수가 *엉뚱한 세션에 보낸 것*이 아니라 *안 보낸 것*이 됩니다. 첨부·캡션·발신자가 그대로 따라가서, 받는 토픽은 처음부터 거기서 친 것과 완전히 같게 받습니다. **켤 것이 없습니다** — 셋이 동시에 참인 방(포럼 그룹 · 상위 토픽 · 봇이 아는 형제 토픽이 하나 이상)에만 적용되므로 DM·일반 그룹·토픽 없는 포럼은 그대로입니다. 설계: [상위 토픽 접수처](docs/design/room-router.md).
...
- **방별 동시 실행** — 방끼리는 **병렬로** 돕니다. DM에서 긴 작업이 돌아도 그룹은 기다리지 않고, 반대도 마찬가지입니다. 방마다 세션이 독립적이라 서로 줄 세울 이유가 없습니다. 한 방 안에서는 여전히 한 번에 하나씩 처리합니다(아래 큐). 예약 작업은 전용 슬롯을 써서 예약끼리는 순서대로, 사용자 방과는 병렬로 돕니다. 로컬 `ctb` 세션은 잡고 있는 그 방만 멈추고 나머지는 그대로 답합니다 — 어느 방인지는 `/local`로 확인하고, 켜둔 채 나왔다면 거기서 끝낼 수 있습니다.
- **연속 메시지 합치기** — 사람은 한 생각을 여러 메시지로 쪼개 보냅니다. "아까 그 버그 말인데" / "테스트부터 돌려봐" / "아 로그도". 첫 줄에 바로 반응하면 나머지는 이미 시작된 작업 뒤에 줄을 서고, 반쪽짜리 맥락으로 돌린 그 실행은 통째로 버려집니다. 그래서 **1초** 동안 다음 메시지를 기다렸다 합쳐서 한 번만 실행합니다(`mergeWindowMs`, `0` 이면 끔). 메시지가 올 때마다 창을 다시 열되 5배에서 끊어서 계속 떠드는 방이 무한정 밀리지 않게 하고, 타이핑 표시는 즉시 띄워 이 침묵이 봇이 죽은 것처럼 보이지 않게 합니다. 텔레그램 상한(4096자)을 넘는 로그를 붙여넣으면 보내는 쪽 클라이언트가 알아서 여러 메시지로 쪼개는데, 첫 조각만 나머지보다 1초 넘게 먼저 도착합니다 — 클라이언트가 첫 조각의 ack 를 받고서야 나머지를 몰아 보내기 때문입니다. 그래서 1초 창으로는 정작 내용이 담긴 뒤 조각들을 대기열로 밀어내고 첫 조각만 혼자 실행됩니다. 이를 막기 위해 길이가 상한에 붙은 메시지는 "뒤가 더 온다"로 읽고 창을 최대 8초로 늘립니다. 마지막 조각은 짧아서 창이 곧바로 기본값으로 돌아오므로, 보통 메시지를 기다리게 하지 않으면서도 붙여넣기는 한 번에 답합니다. 명령어는 지연되지 않습니다 — `/status`·`/stop` 은 그대로 즉시 답하고, 아직 시작 안 한 창은 `/stop` 으로 취소됩니다. 붙잡아 둔 메시지는 창이 열려 있는 동안 `state.json` 에 적어 두고 다음 기동 때 이어서 실행합니다 — 대기열에 밀린 메시지와 달리 ⏳ 안내조차 나가지 않은 상태이고, 텔레그램 Bot API 는 이미 받아간 메시지를 다시 가져올 수 없어서 재시작에 흘리면 영영 사라지기 때문입니다. 재시작 없이 `/mergewindow` 로 조정합니다 — 인자 없이 보내면 현재값과 프리셋 버튼(0.5s / 1s / 2s / 3s / 5s / 끄기 / 기본값)이 나오고, `/mergewindow 2s`·`/mergewindow 2000`·`off`·`default` 도 됩니다. 0.1초~30초를 벗어난 값은 거절하며, 설정값은 `state.json` 에 저장돼 재시작 후에도 유지됩니다.
...
- **다른 방으로 메시지 넘기기** — 방마다 세션이 갈려 있어서 한 방에서 알아낸 걸 옆방은 모릅니다. `/tell <방> <메시지>` 로 메시지 하나를 넘깁니다. `/tell` 만 보내면 이 봇이 아는 방(한 번이라도 대화한 방) 목록이 나오고, 방은 그 목록의 번호나 이름의 일부로 지목합니다 (`큐브기획방 / 마케팅` 처럼 토픽 이름엔 공백이 들어가서 보통 일부만 칩니다). 전달된 메시지는 **대상 방의** 세션·provider·model·plan 고정을 그대로 따라 그 방에서 실행되고, 그 방이 바쁘면 그 방 대기열에 쌓이며, 답도 그 방에 남습니다 — 되돌아오지 않습니다. 에이전트도 답변 끝에 `[[ctb-tell: 방 | 메시지]]` 를 붙여 넘길 수 있는데, 이쪽은 대상 방에서 ✅ 를 눌러야 실행됩니다. 모델이 스스로 다른 방의 토큰을 쓰기 시작하는 자리라서입니다. 다른 방에서 전달받은 메시지는 다시 전달할 수 없어(홉 1), 이 승인과 합치면 사람 없이 도는 전달 고리가 생기지 않습니다. 뮤트된 방(`/*`)에는 넘기지 않습니다. `"roomRelay": false` 로 끌 수 있습니다. 설계: [방 사이 메시지 전달](docs/design/room-relay.md).
...
"schedule": [
  ...
  { "cron": "0 9 * * 1,4", "chat": "-1001234567890", "label": "기획 리포트",
    "prompt": "주 2회 기획 리포트를 써줘" }
]
```
...
- **`chat`**(선택) — 방 키 하나 또는 배열. 적으면 그 작업이 **그 방의 역할로** 돌고(그 방의 `personas` 항목과 그 역할의 `/remember` 규칙, provider·model까지) 결과도 **그 방에만** 갑니다. 안 적으면 역할 없이 돌고 결과는 `allowedChatId` 전체로 뿌려집니다 — 역할이 하나인 봇에는 문제 없지만, **한 봇에 역할을 여럿 두면** 방을 안 적은 작업이 *첫 번째* 역할로 돌면서 모든 방에 뿌려집니다. 기획 리포트를 개발자가 써서 개발 그룹에도 붙는 식입니다. 방 키는 `allowedChatId` 안에 있어야 하고(포럼 토픽 `-100…:11` 은 그룹 id 로 검사), 모르는 방을 적은 작업은 **시작할 때 로그에 에러를 남기고 통째로 버립니다** — 엉뚱한 방으로 나가는 것보다 안 도는 게 낫습니다. 뮤트된 방은 건너뜁니다. **`/cron add` 는 친 방을 `chat` 에 자동으로 적습니다** — 어느 방에서 쳤는지 알고 있으니 모르는 척하고 뿌릴 이유가 없습니다. 일부러 모든 방에 보내고 싶으면 config 에서 `chat` 을 비우면 됩니다. `/cron` 목록은 작업마다 목적지를 같이 보여줍니다.
- `chat` 을 적어도 실행은 여전히 **세션 없이** 예약 슬롯에서 합니다 — 방에서 빌리는 건 역할과 주소지, 대화가 아닙니다. 그래서 그 방의 plan 고정이나 대기열에 걸리지 않습니다.
- **새 세션** — 예약 작업은 **독립된 세션**으로 돌아가서 내 대화 맥락을 오염시키지 않습니다(`state.json`은 내 것 그대로). 전용 슬롯에서 돌기 때문에 사용자 방과는 서로 기다리지 않지만, 예약끼리는 순서대로 돕니다 — 발사 시점에 다른 예약 작업이 아직 돌고 있으면 그 회차는 **건너뜁니다**. 로컬 `ctb` 세션이 락을 잡고 있을 때도 마찬가지로 건너뜁니다.
- **건너뛰면 건너뛰었다고 알립니다** — 결과가 갔을 방에 한 줄 남기고, 원인이 로컬 세션이면 그 자리에서 끝낼 수 있는 버튼을 같이 줍니다. 전에는 로그에만 남아서 로그를 안 보면 회차가 통째로 사라진 걸 알 수 없었습니다. 자주 도는 작업이 도배하지 않도록 알림은 **작업마다 한 시간에 한 번**입니다.
...
**돌고 있는 봇에 일 시키기 — `ctb send`.** 위 명령들은 *터미널에서* Claude 를 돌리는 것이고 봇은
그 일을 모릅니다. `ctb send --chat <방> "<메시지>"` 는 반대로, 로컬 유닉스 소켓으로 **이미 돌고
있는 봇에** 메시지를 넘기고 봇이 그 방에서 처리합니다 — typing 이 돌고, 그 방의 세션·설정으로
실행되고, 답도 그 방에 남습니다. 밖에서 보면 텔레그램으로 직접 친 것과 구별되지 않고, 답은
stdout 으로도 와서 스크립트가 이어 쓸 수 있습니다.

```sh
ctb send --chat 플랜 "0.5.0 릴리스 노트 초안 잡아줘"
```

방은 키 그대로도, 이름의 일부로도 지목합니다. 기본은 대상 방에서 ✅/❌ 로 **승인**을 받습니다 —
부르는 쪽이 에이전트일 수 있는데 프로세스는 사람이 친 `ctb` 와 모델이 부른 `ctb` 를 구분하지
못하므로, 안전한 쪽을 양쪽에 적용합니다. 무인 스크립트는 `--now` 로 건너뛰고, 그 경우에도 방에는
표시됩니다. 자기 `ctb` 세션이 쥐고 있는 방으로는 보내지 않고(봇이 그 방을 미루므로 세션이 끝날
때까지 큐에 남습니다), 이렇게 밀어 넣은 메시지는 다른 방으로 다시 전달되지 않습니다. 봇이 안 떠
있으면 로컬 실행으로 조용히 물러서지 않고 그 자리에서 실패합니다. `ctb` 로 띄운 터미널 세션에는
**이 통로가 있다는 걸 알려줍니다** — 사용법과 넘길 수 있는 방 목록이 시스템 프롬프트에 붙는데,
봇이 실제로 떠 있을 때만이라 봇 없이 터미널만 쓰면 토큰을 내지 않습니다. `"cliDispatch": false` 로
소켓을 안 열 수 있습니다. 설계: [CLI 에서 봇에 일 시키기](docs/design/cli-dispatch.md).
...
역할을 나누는 길은 둘입니다 — **봇 하나에서 방마다 역할을 다르게** 두거나, **역할마다 봇을 따로** 띄우거나.

**방을 역할로 쓰는 쪽**(config 의 `personas`)이 대개 낫습니다. 프로세스도 텔레그램 정체성도 하나이고,
같은 봇이라 `/tell` 로 역할 사이에 일을 넘길 수 있습니다 — 봇을 나누면 그건 **불가능**합니다
(텔레그램 봇은 다른 봇의 메시지를 받지 못합니다).
...
역할은 **세션이 사는 동안 고정**입니다 — 지금까지 오간 말이 전부 그 역할의 것이라서요. 그래서
고르는 자리는 세션 경계뿐이고, 봇이 세 곳에서 버튼을 띄웁니다: 그 방의 **첫 메시지**, **`/newchat`**,
**`/new`**. 안 골라도 그냥 진행되며 목록의 첫 역할로 돕니다. 고르면 그 역할의 프롬프트와
**그 역할만의 `/remember` 메모리**(`memory.dev.md`)가 적용됩니다.

`/status` 는 역할 이름을 한 줄로, `/persona` 는 그 프롬프트 본문까지 보여줍니다. 둘 다 바꾸지는
않습니다. `personas` 가 없으면 아무것도 달라지지 않습니다 — `persona` 는 예전 그대로입니다.

`/sessions` 도 역할을 따릅니다. 이 목록은 디스크의 세션 파일을 프로젝트 단위로 훑어 만들기 때문에,
그냥 두면 **다른 방 세션까지** 다 보입니다 — 기획 세션을 개발방으로 끌어오면 세션 경계 규칙이
조용히 뚫립니다. `personas` 를 넣기 전에 만들어진 세션은 역할 기록이 없어서 **❓** 를 달고 목록에
남고, 고르면 그때부터 그 방 역할로 기록됩니다.

### 역할은 자기 작업 폴더를 가질 수 있습니다 (`dir`)

역할에 `dir` 을 적으면 그 방은 그 폴더에서 돕니다. `projectDir` 기준 상대경로이고 절대경로도
받습니다. 안 적으면 예전처럼 `projectDir` 을 그대로 씁니다.

```jsonc
"projectDir": "/Users/me/code",
"personas": [
  { "id": "dev",   "name": "개발자", "prompt": "…" },
  { "id": "alpha", "name": "알파",   "prompt": "…", "dir": "projects/alpha" },
  { "id": "notes", "name": "노트",   "prompt": "…", "dir": "/Users/me/notes" }
]
```

**봇 하나가 프로젝트 여러 개를 맡는 방법**입니다. 가장 큰 이득은 그 폴더의 `CLAUDE.md` 가 따라온다는
것 — 역할이 *config 의 역할 프롬프트 + 그 폴더의 프로젝트 규칙* 으로 합성됩니다. 작업 폴더에 딸린
것은 전부 방을 따라갑니다: 자식 프로세스의 `cwd`, `/sessions`(세션 목록을 폴더 기준으로 훑으므로
폴더가 다르면 저절로 갈립니다), `.ctb-outbox` 와 `.ctb-jobs`(폴더마다 한 쌍씩 — `/jobs` 는 전부
감시하고 어느 프로젝트 작업인지 표시합니다), 그리고 터미널의 `ctb --chat <방>`.

`projectDir` 하위 폴더를 권장하지만 강제하지는 않습니다. 다만 `.ctb-outbox`·`.ctb-jobs` 가 **작업
폴더마다** 생기므로, 흩어 두면 프로젝트마다 `.gitignore` 를 따로 챙겨야 합니다.

역할의 `dir` 이 없는 폴더를 가리키면 **그 방은 아무것도 실행하지 않고** 그렇게 알립니다. 폴더를
대신 만들어 주지도, 다른 역할의 폴더로 물러서지도 않습니다 — 둘 다 엉뚱한 프로젝트를 조용히 고치는
길입니다. 폴더를 만들거나 `dir` 을 고치고 `/restart` 하면 그대로 되살아납니다.

권한(`permissionMode`·`codexSandbox`)은 아직 전역입니다. 역할별 권한은 별도 작업입니다 —
[docs/design/room-personas.md](docs/design/room-personas.md) 참고.

**역할마다 봇을 따로** 두는 게 맞는 경우도 있습니다. 한 그룹 안에 두 정체성이 동시에 필요할 때,
점진적으로 옮기는 중일 때, `allowedChatId` 를 완전히 갈라야 할 때입니다. 그 구성은 아래입니다.

역할별로 봇을 나눌 때는 코드는 그대로 두고 config만 역할별로 둡니다.
...
- **허가된 채팅** — `allowedChatId`로 허용한 텔레그램 계정에 접근할 수 있는 사람. 폰 잠금과 텔레그램 2FA를 켜두세요. 허용한 방이 **그룹**이면 그 방 참가자 전원이 여기 해당합니다 — 화이트리스트는 사람이 아니라 *방* 단위라, 누군가를 그룹에 초대하는 건 그 사람에게 봇을 넘기는 것과 같습니다. **`/allow` 로 연 방도 똑같습니다** — DM 에서 추가한 방은 `config.json` 에 적은 방과 권한이 완전히 같습니다.
...
두 가지가 돕니다. **smoke** 는 CLI 파일에 `node --check` 를 걸고 두 바이너리가 버전을 출력하는지 봅니다. 그다음 `tests/*.test.mjs` 가 동작 스위트를 돌립니다.

`bot.mjs` 는 export 가 없는 단일 파일이라, 스위트는 **소스에서 블록을 떼어내** 스텁 위에서 돌립니다 — 복사본을 두면 코드가 바뀌어도 테스트는 옛것을 통과시키기 때문입니다. 대가는 앵커가 밀릴 수 있다는 것인데, 밀리면 `cut()` 이 **앵커 이름을 대고** 실패합니다(예전에는 엉뚱한 `SyntaxError` 로 죽어서 원인을 못 찾았습니다). 스위트마다 프로세스가 따로라 하나가 깨져도 나머지는 보입니다.

CI는 Node 18, 20, 22에서 같은 검사를 실행합니다.
