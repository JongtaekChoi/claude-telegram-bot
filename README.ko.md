...
| `/persona [역할]` | 이 방이 어떤 역할로 도는지. 인자 없이 보내면 역할 목록. 바꾸는 건 세션 경계에서만 — `/new` 를 먼저 |
...
방마다 `/persona <역할>` 로 고르면 그 역할의 프롬프트와 **그 역할만의 `/remember` 메모리**
(`memory.dev.md`)가 적용됩니다. 역할은 세션이 사는 동안 고정입니다 — 지금까지 오간 말이 전부 그
역할의 것이라서요. 그래서 `/persona` 는 세션 경계에서만 먹고, 대화가 진행 중이면 `/new` 를 먼저
보내야 합니다. 안 고른 방은 목록의 첫 항목으로 돕니다. `personas` 가 없으면 아무것도 달라지지
않습니다 — `persona` 는 예전 그대로 동작합니다.
