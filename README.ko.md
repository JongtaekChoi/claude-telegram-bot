...
| `/rooms [rm <번호>]` | 이 봇이 아는 방 목록 — `/tell` 대상 · `/cron` 목적지 · 포럼 접수처 버튼이 다 이 목록을 씁니다. 방은 처음 말을 걸면 스스로 등록되고 지우는 길이 없어 쌓이는데, `rm` 으로 안 쓰는 것을 정리합니다 (`3`, `3 5 7`, `3-9`). 지워도 대화 기록은 남습니다 — 그 방이 어느 세션이었는지만 잊고, 그 방에서 `/sessions` 로 다시 고를 수 있습니다 |
