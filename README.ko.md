...
| `/rooms` | 이 봇이 아는 방 목록 · `rm <번호>` 로 정리 · `sweep` 으로 지워진 토픽 청소 → [아래 설명](#방-목록-정리-rooms) |
...
방은 거기서 봇에게 말을 한 번 걸면 스스로 등록됩니다. 지우는 길이 없어서 `/newchat` 으로
만들었다 버린 토픽이 목록에 계속 남는데, 그 목록이 곧 `/tell` 의 대상이고 `/cron` 결과가
가는 곳이며 포럼 접수처가 내주는 버튼입니다. 죽은 토픽 하나가 셋을 같이 어지럽힙니다.

```
/rooms                 번호 붙은 목록 · ⏳ 작업 중 · ← 지금 이 방
/rooms rm 3            하나 빼기 — 3 5 7 · 3-9 도 가능
/rooms sweep           텔레그램에서 지운 토픽을 찾아 목록에서 빼기
```

**목록에서 뺀다고 나눈 대화가 없어지지는 않습니다.** 기록은 `~/.claude/projects/` 에 그대로
있고, 그 방에서 `/sessions` 로 다시 고르면 이어집니다. 잃는 것은 방과 세션의 연결, 그리고
그 방에서 고른 역할입니다. 누군가 그 방에서 다시 말을 걸면 새 세션으로 다시 등록됩니다.

두 방은 지울 수 없습니다. **지금 이 방** — 지워도 다음 메시지에 곧바로 되살아나는데 그 사이
세션 연결만 끊깁니다. 그리고 **작업 중인 방** — 답이 돌아올 자리가 없어집니다. 범위에 이 둘이
섞이면 일부만 지우지 않고 통째로 거절합니다. 절반만 지우면 남은 번호가 밀려서, 이어서 친
명령이 엉뚱한 방을 지우기 때문입니다.

<details>
<summary><b><code>sweep</code> 이 메시지를 보내야 하는 이유</b></summary>

텔레그램은 토픽이 지워져도 봇에게 알려주지 않습니다. `getForumTopic` 같은 조회 메서드가 없고,
서비스 메시지도 토픽 *생성* · *수정* · *닫힘* · *다시 열림* 뿐 "삭제"는 없습니다.
`sendChatAction` 도 소용없습니다 — 없는 토픽에 보내도 `ok: true` 가 돌아옵니다.

살아 있는 토픽과 지워진 토픽을 가르는 응답은 진짜 메시지뿐입니다:

```
sendMessage(message_thread_id: <지워진 토픽>)
  → 400  Bad Request: message thread not found
```

그래서 `sweep` 은 알림을 끈 점 하나를 보내고 곧바로 지웁니다. 살아 있는 토픽에서는 잠깐
깜빡일 수 있지만 남는 것은 없습니다.

훑는 대상은 포럼 토픽뿐입니다. 그룹 자체나 DM 은 건드리지 않습니다 — 거기서 깜빡이면 사람들이
보는 자리이기 때문입니다. 그리고 **애매한 응답은 절대 삭제로 치지 않습니다.** 플러드 제한,
네트워크 실패, 권한 오류가 나면 그 방은 그대로 두고 보고에 적습니다. 잠깐 끊긴 것을 죽은
토픽으로 오해하면 지금 쓰는 방을 지우게 됩니다.
</details>
