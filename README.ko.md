...
| `personas` | (선택) 역할 **목록**. 봇 하나가 여러 역할을 맡습니다. 항목마다 `id`(`[a-z0-9-]`, 파일명이 됩니다)와 `prompt` 가 필요하고 `name` 은 선택. 방은 `/persona` 로 고르며, 안 고른 방은 첫 항목으로 돕니다. 아래 참고 |
...
| `/sessions` | 이 프로젝트의 지난 세션 목록(활성 provider 기준) · 골라서 이어가기 (🔒 다른 방이 쓰는 중, 💻 터미널에서 열려 있음 — 둘 다 못 고름. `personas` 를 쓰면 이 방 역할의 세션만 보이고 ❓ 는 역할 생기기 전 세션) |
