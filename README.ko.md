...
- **`chat`**(선택) — 방 키 하나 또는 배열. 적으면 그 작업이 **그 방의 역할로** 돌고(그 방의 `personas` 항목과 그 역할의 `/remember` 규칙, provider·model까지) 결과도 **그 방에만** 갑니다. 안 적으면 역할 없이 돌고 결과는 `allowedChatId` 전체로 뿌려집니다 — 역할이 하나인 봇에는 문제 없지만, **한 봇에 역할을 여럿 두면** 방을 안 적은 작업이 *첫 번째* 역할로 돌면서 모든 방에 뿌려집니다. 기획 리포트를 개발자가 써서 개발 그룹에도 붙는 식입니다. 방 키는 `allowedChatId` 안에 있어야 하고(포럼 토픽 `-100…:11` 은 그룹 id 로 검사), 모르는 방을 적은 작업은 **시작할 때 로그에 에러를 남기고 통째로 버립니다** — 엉뚱한 방으로 나가는 것보다 안 도는 게 낫습니다. 뮤트된 방은 건너뜁니다. `/cron add` 로 그때그때 추가한 작업에는 `chat` 이 없습니다.
