# Claude Code Instructions

## 작업 범위
`bot.mjs`(단일 파일, 의존성 0)와 `ctb.mjs`, `README`(영/국문)를 주로 다룬다.
변경 시 기존 코드 스타일을 따르고, `config.json` / `state.json` 같은 민감 파일은 건드리지 않는다.

## 작업 대기열은 `docs/BACKLOG.md` 하나다
"다음에 뭘 할까"는 [docs/BACKLOG.md](docs/BACKLOG.md) 에서 찾는다. 세션은 끊기고 대화는 스크롤
밖으로 밀려나므로, 대기열을 대화에만 두면 이미 끝난 걸 다음 작업이라고 제안하게 된다(실제로 그랬다).

- 작업을 끝냈으면 그 항목을 **지운다.** 이력은 `CHANGELOG.md` 와 `git log` 에 있다.
- 커밋은 했지만 실봇으로 확인 안 했으면 **검증 빚**으로 옮긴다 — 배포 전에 비어 있어야 하는 칸이다.
- 새 요청·버그 신고를 받았는데 지금 안 할 거면 그 자리에서 적어 둔다.
- 안 하기로 결론 난 것도 **"안 하기로 한 것"에 남긴다.** 안 남기면 몇 달 뒤에 같은 제안이 또 올라온다.

## 기능을 고치기 전에 그 기능의 설계 문서를 읽는다
`docs/design/` 이 다루는 영역을 손대면 해당 문서를 먼저 읽고, 구현이 기존 결정을 뒤집었으면
코드와 **같은 커밋에서** 문서도 고친다. 뒤집힌 결정은 지우지 말고 왜 뒤집혔는지 덧붙인다.

- 그룹·방별 세션과 동시 실행: `docs/design/group-chat-multi-user.md`
- Claude ↔ Codex 인수인계: `docs/design/provider-session-handoff.md`
- 방 사이 메시지 전달(`/tell`): `docs/design/room-relay.md`
- 오너 DM 어드민과 설정 쓰기 경계: `docs/design/owner-admin.md`
- CLI 에서 봇에 일 시키기: `docs/design/cli-dispatch.md`
- 방별 페르소나(한 폴더 한 봇): `docs/design/room-personas.md`
- 잘못 보낸 방을 막는 접수처(`/router`): `docs/design/room-router.md`

## 명령어를 바꿨으면 안내 문구도 같이 바꾼다
문법(인자·옵션)을 손댔으면 그 명령을 설명하는 자리를 **전부** 찾아 고친다. 한 군데만 고치면
사용자는 옛 문법을 보고 새 기능이 있는 줄 모른다. 실제로 `/memory rm` 에 범위를 붙였을 때
여섯 군데가 갈렸다. 찾을 자리:

- `<명령>Show` / `<명령>List` — **목록 아래 붙는 안내. 가장 많이 읽히는 자리이고 가장 잘 빠뜨린다.**
- `<명령>Usage` — 잘못 입력했을 때
- `helpText` — 영/한 각각 (한 파일에 두 벌 있다)
- `setMyCommands` 배열 — 텔레그램 `/` 자동완성 설명. 영/한 각각
- 그 밖의 유도 문구 (`memoryCrowded` 처럼 다른 명령을 권하는 문장)
- `README.md` · `README.ko.md` 명령어 표

`grep -n "<명령>" bot.mjs` 로 훑고, 문구가 영·한 쌍으로 있는지 확인한다 — 한쪽만 고치는 게 흔한 실수다.

## 배포 규칙
`git push` 또는 `npm publish` 전에 반드시 사용자에게 먼저 확인한다.
버전 번호 변경(package.json)과 CHANGELOG 업데이트는 커밋 전에 같이 처리한다.
배포하면 `git tag v<버전>` 을 릴리스 커밋에 붙이고 푸시한다(lightweight, 기존 관례).

**minor 를 올릴 자리를 놓치지 않는다.** 규칙이 없어서 patch 가 `0.3.41`·`0.4.13` 까지 흘러갔고,
그 결과 **버전 번호가 정보를 잃었다** — 큰 릴리스와 오타 수정이 똑같이 생겼다. 아래 중 하나라도
있으면 **minor**:

- `config.json` 에 **새 키**가 생긴다 (`personas`, `provider`, `cliDispatch` …)
- **새 명령**이 생긴다 (텔레그램 `/…` 또는 `ctb <하위명령>`)
- 쓰는 방식에 **새 축**이 생긴다 — 0.4.0 은 provider(Claude↔Codex), 0.4.13 은 방별 역할이 그랬다
  (0.4.13 은 이 규칙이 없어서 patch 로 나갔다. 다음부터 이러지 않으려고 적어 둔다)

나머지는 patch — 버그 수정, 문구, 기존 기능의 개선.

**동작을 고쳤으면 `tests/` 에 스위트를 남긴다.** `bot.mjs` 는 export 가 없으므로 소스에서 블록을
떼어(`tests/helpers/extract.mjs` 의 `cut`) 스텁 위에서 돌린다 — 복사본을 두면 코드가 바뀌어도
테스트는 옛것을 통과시킨다. **세션 스크래치패드에 두지 않는다** — 2026-09-02 에 세션이 끊기면서
다섯 스위트가 한꺼번에 증발했다. `npm test` 가 smoke 와 함께 전부 돌린다.

**`npm publish` 전에는 실제 봇으로 코드 테스트가 필수다.** `npm test`(문법 + 동작 스위트)만으로는 부족하다 —
실제로 텔레그램 메시지를 보내서 응답이 오는지 확인한다. 특히 `send()`, 메시지 큐, 인라인 버튼처럼
텔레그램 왕복이 있는 코드를 건드렸다면 반드시 실동작 확인 후 배포할 것.
(계기: [docs/incidents/2026-07-03-plan-send-silent-failure.md](docs/incidents/2026-07-03-plan-send-silent-failure.md) —
제너레이터 함수를 배열처럼 인덱싱해 `send()`가 통째로 무응답이 됐는데, 실사용 테스트 없이 배포되어 발견이 늦어짐.)

**테스트는 publish 전에 해야 하므로 레지스트리에서 받을 수 없다.** 배포될 그대로를 tarball 로
만들어 전역 설치한 뒤 실봇을 돌린다 — `npm i -g .` (폴더 직접)는 심볼릭 링크라 레포 파일을 다
보게 돼서 `package.json` 의 `files` 에서 빠뜨린 파일을 못 잡는다. tarball 은 잡는다.

```
npm pack --pack-destination <tmp>
npm i -g <tmp>/claude-telegram-bot-<버전>.tgz     # ← --prefix 주의, 아래 참고
launchctl kickstart -k gui/$(id -u)/<label>       # 실봇으로 확인
# 통과하면 그때 npm publish
```

전역 설치 위치와 launchd 봇들의 배치는 환경마다 다르다(이 머신은 node 두 버전에 슬롯이 갈려
있어 `--prefix` 를 명시해야 한다). 봇을 재시작할 때 `bootout` → `bootstrap` 을 잇달아 하면
`Input/output error` 로 실패하니 텀을 두거나 `kickstart -k` 를 쓴다.
